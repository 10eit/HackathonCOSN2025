...
def extract_whisper_features(
    ...
    返回:
        features: 包含多种特征的字典
            ...
            - embedding_unencode: 未经过编码器的嵌入表示 (shape: [1500, n_features])
            - encoder_final_hidden: 编码器隐藏层特征 (shape: [1500, n_features])
    ...
    with torch.no_grad():  # 禁用梯度计算，节省内存
        # 编码阶段（获取编码器特征）
        encoder_output, embed_unencoded = model.encoder(mel)  # shape: [1, 1500, n_features]

        # 解码阶段（获取logits）
        decoder_input = torch.tensor([[whisper.tokenizer.get_tokenizer(multilingual=True, language="zh").sot]]).to(device)  # 起始token
        logits, decoder_hidden = model.decoder(decoder_input, encoder_output)  # shape: [1, 1, vocab_size]
